    ...
    try:
        ...
        for attachment in tnef.attachments:
            if isinstance(attachment.name, bytes):
                attachment_path = os.path.join(output_dir, attachment.name.decode('utf-8', 'ignore'))
...
def main():
    parser = argparse.ArgumentParser(description="Extract attachments and email body from Winmail.dat files using tnefparse.")
    parser.add_argument("winmail_dat_file", help="Path to the Winmail.dat file.")
    parser.add_argument("-o", "--output_dir", help="Output directory for attachments (optional).")
