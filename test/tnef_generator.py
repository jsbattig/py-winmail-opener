...
class TNEFGenerator:
    ...
    def __init__(self, resources_dir):
        ...
        try:
            ...
            subprocess.run(['which', 'mpack'],
                          stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE,
                          check=True)
        except (FileNotFoundError, subprocess.CalledProcessError):
            raise RuntimeError("mpack utility is required. Install with 'brew install mpack' on macOS")
    
    def create_winmail_dat(self, output_path, body_text, subject=None, 
                          sender=None, recipient=None, date=None,
                          attachments=None, html_body=False):
        """
        ...
        try:
            ...
            try:
                ...
                result = subprocess.run(['mpack', '-o', winmail_path, eml_path],
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE,
                                      check=True,
                ...
                if os.path.exists(winmail_path):
                    print(f"winmail.dat file created: {os.path.getsize(winmail_path)} bytes")
                else:
    ...
    def create_metadata_winmail(self, output_path):
        ...
        body_text = "This email contains metadata like sender, recipient, subject, and date."
        return self.create_winmail_dat(
            output_path=output_path,
            body_text=body_text,
