...
class MockTNEF:
    ...
    def __init__(self, body=None, rtfbody=None, htmlbody=None, attachments=None, **metadata):
        self.body = body
...
class WinmailOpenerTests(unittest.TestCase):
    ...
    def test_body_only_extraction(self):
        ...
        with open(output_html, 'r') as f:
            content = f.read()
            self.assertIn("This is a simple test email body with no special formatting or attachments.", content)
        
        # Verify no attachments were extracted
        downloads_dir = os.path.join(self.output_dir, 'Downloads')
        self.assertEqual(len(os.listdir(downloads_dir)), 0, 
    ...
    def test_attachments_extraction(self):
        ...
        downloads_dir = os.path.join(self.output_dir, 'Downloads')
        self.assertTrue(os.path.exists(downloads_dir), "Downloads directory should exist")
        
        extracted_files = os.listdir(downloads_dir)
        self.assertEqual(len(extracted_files), 2, "Two attachments should have been extracted")
        
        # Check for specific attachment files
        self.assertTrue(any("sample_image.txt" in f for f in extracted_files),
                       "Image attachment should be extracted")
        self.assertTrue(any("sample_document.txt" in f for f in extracted_files),
    ...
    def test_command_line_interface(self):
        ...
        with unittest.mock.patch('winmail_opener.extract_winmail_dat') as mock_extract:
            ...
            script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'winmail_opener.py'))
            ...
            self.assertEqual(result.returncode, 0,
                            f"Process failed with stderr: {result.stderr}")
